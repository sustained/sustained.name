...
export async function asyncData({ app, store, params, error }) {
  ...
  try {
    const markdown = await app.loadMarkdown(
      `${entitiesName}/${entityValue}/content.md`
    );
  ...
  computed: {
    meta() {
      ...
      if (Object.keys(this.$route.params).length > 0) {
        ...
        entityMetadata = this.$store.state["dynamic-markdown"][entitiesName][
          entityValue
        ];
      }
      ...
      return {
        name:
          entityMetadata.name ||
          entityValue.substr(0, 1).toUpperCase() + entityValue.substr(1),
...
export const setHead = {
  head() {
    ...
    return {
      title: `${this.meta.name} - ${entitiesName.substr(0, 1).toUpperCase() +
        entitiesName.substr(1)}`,
